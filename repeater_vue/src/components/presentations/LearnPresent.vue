<template>
  <div class="container">
    <div v-show="!changePhase">
      <div class="btn">
        <button @click="deleteCard(card.card_id)" class="button" id="btn-del">
          Usuń
        </button>
      </div>
      <div class="test">
      ...
      <div>
        <div class="btn">
          <button
            @click="editCard(wordIdInDataBase)"
            class="button-1"
            id="btn-edit"
          >
            Edytuj
  ...
  methods: {
    ...
    whatNote(nota) {
      switch (nota) {
        ...
        case 3: // Well known card
          this.ocena = 3;
          console.log(
            "czy jest overlearning? ",
            this.$store.getters["option/getLearningDayLimit"]
          );
          this.knownNoting();
          break;
      }
    ...
    knownNoting() {
      ...
      if (this.wordsList[this.index]["learned_at"]) {
        ...
        console.log(
          "czy jest overlearning? ",
          this.$store.getters["option/getOverlearning"]
        );
        ...
        if (this.$store.getters["option/getOverlearning"]) {
          ...
          console.log(
            "Obliczanie ocerlearningu wszystkie próby:",
            this.wordsList[this.index]["tries"]
          );

          console.log(
            "Obliczanie ocerlearningu",
            Math.floor(this.wordsList[this.index]["tries"] / 2)
          );
    ...
    unknownNoting() {
      ...
      if (
        this.wordsList[this.index]["tries"] >=
        this.$store.getters["getMaxNumTries"]
      ) {
    ...
    easyNoting() {
      ...
      this.wordsList[this.index]["learned_at"] = date.getToday();
      console.log(
        "this.wordsList[this.index][learned_at]",
        this.wordsList[this.index]["learned_at"]
      );
      //setting today's date as last repeat date:
      this.wordsList[this.index]["last_repeat"] = date.getToday();
      console.log(
        "this.wordsList[this.index][last_repeat]",
        this.wordsList[this.index]["last_repeat"]
      );
      // ustawienie powtórki za 5 dni
      this.wordsList[this.index]["next_repeat"] = date.getDatePlusDays(
    ...
    newWordPresentation() {
      if (this.index == this.listLength) {
        ...
        if (
          this.wordsList[this.index]["learned_at"] &&
          !this.wordsList[this.index]["overlearningAmount"]
        ) {
    ...
    updateDataCardInDb(row) {
      http
        .post("learning/updateCard/" + this.wordIdInDataBase, row)
        .then((result) => {
