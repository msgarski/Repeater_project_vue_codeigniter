<template>
  ...
  <div class="container">
    ...
    <form @submit.prevent="addChanges" class="form">
      <div class="label">
        <label for="learningBatchLimit"
          >Limit słów do nauki w jednej serii</label
        >
        <select
          name="learningBatchLimit"
          v-model="learningBatchLimit"
          id="learningBatchLimit"
        >
          <option
            v-for="limit in learningAmountBatch"
            :key="limit.id"
            :learningLimit="limit.value"
            >{{ limit }}
          </option>
        </select>
      </div>
      ...
      <div class="label">
        ...
        <select name="learningLimit" v-model="learningLimit" id="learningLimit">
          <option
            v-for="limit in learningLimitsTable"
            :key="limit.id"
            :learningLimit="limit.value"
            >{{ limit }}
          </option>
      ...
      <div class="label">
        ...
        >
          <option
            v-for="limit in repeatLimitsTable"
            :key="limit.id"
            :learningLimit="limit.value"
            >{{ limit }}
          </option>
      ...
      <div class="label">
        ...
        <select
          name="fastRepeatBatch"
          v-model="fastRepeatBatch"
          id="fastRepeatBatch"
          class="input"
        >
          <option
...
export default {
  ...
  created() {
    ...
    this.learningBatchLimit = this.$store.getters[
      "option/getLearningBatchLimit"
    ];
  ...
  methods: {
    ...
    addChanges() {
      ...
      http
        .post("/options/updateOptions", pack)
        .then((response) => {
        ...
        .then(() => {
          ...
          this.$store.dispatch(
            "option/setFastRepeatBatch",
            pack.fastRepeatBatch
          );
